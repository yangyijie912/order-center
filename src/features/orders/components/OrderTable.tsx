  ...
  return (
    <div style={{ border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {loading ? (
            <tr><td style={td} colSpan={6}>加载中...</td></tr>
          ) : orders.length === 0 ? (
            <tr><td style={td} colSpan={6}>暂无数据</td></tr>
          ) : (
            orders.map((o) => (
              <tr key={o.id} style={{ borderTop: '1px solid #eee' }}>
                <td style={td}>{o.id}</td>
                <td style={td}>{o.userName}</td>
                <td style={td}>{o.currency} {o.amount.toFixed(2)}</td>
                <td style={td}>{statusLabel(o.status)}</td>
                <td style={td}>{new Date(o.createdAt).toLocaleString()}</td>
                <td style={td}>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <button onClick={() => onView(o)} style={btn}>详情</button>
                    <button
                      onClick={() => onCancel(o)}
                      style={btn}
